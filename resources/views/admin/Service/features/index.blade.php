...
<section class="content-header">					
    <div class="container-fluid my-2">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>{{$service->title}}->>Features</h1>
            </div>
            <div class="col-sm-6 text-right">
                <a href="{{route('service_management.features.create',$service->id)}}" class="btn btn-success"><i class="fas fa-plus"></i>New Features</a>
                  ...
                  <form action="" method="GET">
                    <div class="input-group input-group" style="width: 250px;">
                        ...
                        <div class="input-group-append">
                          ...
                          <a href="{{route('service_management.features',$service->id)}}" class="btn btn-default">
                ...
                <table class="table table-hover text-nowrap">
                    ...
                    <tbody>
                        ...
                        @forelse ($features as $key => $item)
                            <tr>
                                <td>{{$key+1}}</td>
                                <td><img src="{{asset($item->image)}}" alt="no-image" width="85px"></td>
                                <td>{{$item->title}}</td>
                                <td>
                                ...
                                <td>
                                    <a href="{{route('service_management.features.edit',[$service->id,$item->id])}}">
                                        <svg class="filament-link-icon w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                        </svg>
                                    </a>
                                    <a href="javascript:void();" class="delete-btn text-danger w-4 h-4 mr-1" data-id="{{$item->id}}">
    ...
    <script>
        $(document).ready(function(){
            $('.status-toggle').on('click',function(){
                var featuresId = $(this).data('id');
                var currentStatus = $(this).data('status');
                var newStatus = currentStatus == 1 ? 0 : 1; // Toggle status

                $.ajax({
                    url: "{{ route('service_management.features.status') }}", // Update this route
                    type: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        id: featuresId,
        ...
        $('.delete-btn').click(function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '../features/delete/' + itemId; // Replace '/delete/' with your actual delete route
                }
            });
        });
    });
